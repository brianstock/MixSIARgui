...
build_source_win <- function(){
  ...
  raw_data <- gWidgets::gbutton(
    ...
    handler = function(h, ...){
      gWidgets::gfile(
        ...
        handler = function(h, ...){
          tryCatch(   # reads source data into 'SOURCE'
            ...
            gWidgets::add(grp_raw,gWidgets::gimage(system.file("extdata", "check.png", package = "MixSIARgui")))
            ...
            error = function(e){
              ...
              gWidgets::add(grp_raw,gWidgets::gimage(system.file("extdata", "red_x.png", package = "MixSIARgui")))
            }
          )
        }
      )
      ...
      source <- MixSIAR::load_source_data(mixsiar$source_filename,source_random_effects,conc_dep,data_type,mix)
      ...
      gWidgets::gfile(
        ...
        handler = function(h, ...){
          tryCatch(   # reads source data into 'SOURCE'
            ...
            the_data <- do.call(h$action, list(h$file, header=T))
            assign(data_frame_name, the_data, envir = mixsiar)
            gWidgets::add(grp_means_sds,gWidgets::gimage(system.file("extdata", "check.png", package = "MixSIARgui")))
            source_filename <- h$file; assign("source_filename",source_filename,envir=mixsiar)
            },
            error = function(e){
              gWidgets::svalue(mixsiar$status_bar) <- "Could not load data"
              gWidgets::add(grp_means_sds,gWidgets::gimage(system.file("extdata", "red_x.png", package = "MixSIARgui")))
            }
          )
        }
      )
  ...
  btn_close_source <- gWidgets::gbutton(
    ...
    handler	= function(h, ...){
      ...
      if(exists("test")){
        gWidgets::add(mixsiar$grp_source,gWidgets::gimage(system.file("extdata", "check.png", package = "MixSIARgui")))
        ...
        gWidgets::add(mixsiar$grp_source,gWidgets::gimage(system.file("extdata", "red_x.png", package = "MixSIARgui")))
      }
    }
  )
  gWidgets::addSpring(source_grp_all)
