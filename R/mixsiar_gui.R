...
#' and GTK+. See README for install instructions:
#' \url{https://github.com/brianstock/MixSIARgui}
#'
#' \code{mixsiar_gui} calls most of the other \code{MixSIAR} functions:
#'  \itemize{
#'    \item \code{MixSIAR::load_mix_data()}
#'    \item \code{MixSIAR::load_source_data()}
#'    \item \code{MixSIAR::load_discr_data()}
#'    \item \code{MixSIAR::plot_data()}
#'    \item \code{MixSIAR::plot_prior()}
#'    \item \code{MixSIAR::write_JAGS_model()}
#'    \item \code{MixSIAR::run_model()}
#'    \item \code{MixSIAR::output_JAGS()}
#'  }
#'
...
#' mixsiar_gui()
#'
#' @seealso \code{MixSIAR::load_mix_data()} loads the mixture data file,
#' @seealso \code{MixSIAR::load_source_data()} loads the source data file,
#' @seealso \code{MixSIAR::load_discr_data()} loads the TDF data file,
#' @seealso \code{MixSIAR::plot_data()} creates an isospace plot,
#' @seealso \code{MixSIAR::plot_prior()} plots your prior and the uninformative prior,
#' @seealso \code{MixSIAR::write_JAGS_model()} creates a JAGS model file (.txt),
#' @seealso \code{MixSIAR::run_model()} sets up JAGS objects and calls JAGS to run
#'          the model,
#' @seealso \code{MixSIAR::output_JAGS()} processes the JAGS output (prints/saves
...
mixsiar_gui <- function(){
  ...
  if(requireNamespace("gWidgets", quitely=TRUE)){
          ...
          handler = function(h, ...)
          {
            tryCatch(   # reads discrimination/fractionation/enrichment means data into 'FRAC'
              ...
              error = function(e){
                ...
                gWidgets::add(grp_frac,gWidgets::gimage(system.file("extdata", "red_x.png", package = "MixSIARgui")))
              }
            )
          }
        )
        discr <- MixSIAR::load_discr_data(mixsiar$discr_filename, mixsiar$mix)
        assign("discr", discr, envir = mixsiar)
        gWidgets::add(grp_frac,gWidgets::gimage(system.file("extdata", "check.png", package = "MixSIARgui")))
      }
    )
    #############################################################################
    ...
    plot_button <- gWidgets::gbutton(
      ...
      expand = TRUE,
      handler = function(h, ...){
        MixSIAR::plot_data(gWidgets::svalue(mixsiar$plot_filename), gWidgets::svalue(mixsiar$plot_save_pdf), gWidgets::svalue(mixsiar$plot_save_png), mixsiar$mix,mixsiar$source,mixsiar$discr)
    ...
    plot_button <- gWidgets::gbutton(
      ...
      handler = function(h, ...){
        ...
        MixSIAR::plot_prior(alpha.prior,mixsiar$source,gWidgets::svalue(plot_save_pdf_prior),gWidgets::svalue(plot_save_png_prior),gWidgets::svalue(plot_filename_prior))
        }
          )
    ...
    go_button <- gWidgets::gbutton(text="RUN MODEL", cont=grp_run_model, expand=TRUE,
                                   handler = function(h, ...){
                                     ...
                                     MixSIAR::write_JAGS_model("MixSIAR_model.txt", resid_err, process_err, mixsiar$mix, mixsiar$source)

                                     run <- gWidgets::svalue(mixsiar$mcmc_run)
                                     jags.1 <- MixSIAR::run_model(run, mixsiar$mix, mixsiar$source, mixsiar$discr, "MixSIAR_model.txt", alpha.prior, resid_err, process_err)
                                     assign("jags.1",jags.1,envir=mixsiar)

                                     test <- get("jags.1",envir=mixsiar)
                                     if(exists("test")){
                                       gWidgets::add(grp_run_model,gWidgets::gimage(system.file("extdata", "check.png", package = "MixSIARgui")))
    ...
    output_button <- gWidgets::gbutton(text="Process output", cont=grp_output, expand=TRUE,
                                       handler = function(h, ...){
                                         output_options <- list(gWidgets::svalue(mixsiar$summary_save),     # Save the summary statistics as a txt file?
                                                                ...
                                                                FALSE,
                                                                diag_save_ggmcmc=gWidgets::svalue(mixsiar$diag_save))
                                         MixSIAR::output_JAGS(mixsiar$jags.1, mixsiar$mix, mixsiar$source, output_options)
                                       })
