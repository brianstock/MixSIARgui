...
#' \code{MixSIAR::load_mix_data()} when the user clicks the "I'm finished" button.
#'
...
#'
#' @seealso \code{MixSIAR::load_mix_data()}, which is run when the "Load MIX data"
    ...
    gWidgets::gfile(
      ...
      handler= function(h, ...)
      {
        tryCatch(
        {
          ...
          gWidgets::add(mix_grp_cons,gWidgets::gimage(system.file("extdata", "check.png", package = "MixSIARgui")))
        ...
        error = function(e){
          ...
          gWidgets::add(mix_grp_cons,gWidgets::gimage(system.file("extdata", "red_x.png", package = "MixSIARgui")))
        }
        )
      }
    )
...
btn_close <- gWidgets::gbutton(
  ...
  handler	= function(h, ...){
    ...
    mix <- MixSIAR::load_mix_data(mixsiar$mix_filename, iso_names, factors, fac_random, fac_nested, cont_effects)
...
#           }
    ...
    if(exists("test")){
      gWidgets::add(mixsiar$grp_cons,gWidgets::gimage(system.file("extdata", "check.png", package = "MixSIARgui")))
      ...
      gWidgets::add(mixsiar$grp_cons,gWidgets::gimage(system.file("extdata", "red_x.png", package = "MixSIARgui")))
    }
